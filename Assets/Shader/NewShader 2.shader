Shader "Eleanor/NewShader 2" {
//add gaussian distribution
	Properties {
		_MainTint ("Global Tint", Color) = (0.55, 1.0, 0.55, 1)
		_SubTint ("Global Tint", Color) = (0.55, 1.0, 0.55, 1)
		_BumpMap ("Normal Map", 2D) = "bump" {}
		_BitMap ("Bit Map", 2D) = "grey" {}
		_Check ("Check", Range(0, 1)) = 0
		_FresnelPower ("Fresnel Power", Range(0, 10)) = 3
		_FresnelPowerRow ("Fresnel Power Row", Range(0, 10)) = 3
		_FresnelPowerCol ("Fresnel Power Col", Range(0, 10)) = 3
		_Roughness ("Roughness", Range(0, 1)) = 0.5
		_GaussianSigma ("Gaussian Sigma", Range(0, 1)) = 0.5
		_GaussianSigmaRow ("Gaussian Sigma Row", Range(0, 1)) = 0.5
		_GaussianSigmaCol ("Gaussian Sigma Col", Range(0, 1)) = 0.5
	...
	SubShader {
		...
		float _GaussianSigmaRow;
		float _GaussianSigmaCol;
		float _kd;
		float _A;
		...
		inline fixed4 LightingCloth(SurfaceOutput s, fixed3 lightDir, half3 viewDir, fixed atten) {
			...
			float gS = exp(-pow(thetaH / _Roughness, 2));
			...
			float gV = exp(-pow(thetaH / _GaussianSigma, 2));
